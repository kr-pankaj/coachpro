<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('superadmin.kb-articles.update', $kbArticle) }}" method="POST" class="space-y-8">
                @csrf @method('PUT')
                ...
                <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 p-10 shadow-sm">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                        <div class="md:col-span-2">
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full text-lg font-bold" value="{{ $kbArticle->title }}" required />
                        </div>
                        ...
                        <div>
                            ...
                            <select name="kb_category_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-2xl shadow-sm" required>
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ $kbArticle->kb_category_id == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="flex items-center gap-8 pt-6">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_published" class="sr-only peer" {{ $kbArticle->is_published ? 'checked' : '' }}>
                            ...
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_internal_only" class="sr-only peer" {{ $kbArticle->is_internal_only ? 'checked' : '' }}>
