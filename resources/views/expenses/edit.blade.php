<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="font-black text-2xl text-gray-900 dark:text-white tracking-tight">Edit Expense</h2>
                <p class="text-sm text-gray-500 mt-1">Update the details of this expense record.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-gray-700 p-10">
                ...
                <form method="POST" action="{{ route('expenses.update', $expense) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-[10px] font-black text-gray-500 uppercase tracking-widest block mb-2">Linked Batch <span class="text-gray-300 font-normal normal-case">(optional)</span></label>
                            <select name="batch_id"
                                    class="w-full border border-gray-200 dark:border-gray-600 rounded-2xl px-5 py-3.5 text-sm font-medium text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all">
                                <option value="">Institute Overhead (no batch)</option>
                                @foreach($batches as $b)
                                    <option value="{{ $b->id }}" {{ old('batch_id', $expense->batch_id) == $b->id ? 'selected' : '' }}>{{ $b->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                                  class="w-full border border-gray-200 dark:border-gray-600 rounded-2xl px-5 py-3.5 text-sm font-medium text-gray-900 dark:text-white dark:bg-gray-700 focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all resize-none">{{ old('notes', $expense->notes) }}</textarea>
                    </div>
