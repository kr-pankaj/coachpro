                    ...
                    <form method="POST" action="{{ route('students.update', $student) }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="batch_ids" name="batch_ids[]" class="block mt-1 w-full" multiple>
                                    @foreach($batches as $batch)
                                        <option value="{{ $batch->id }}" {{ (is_array(old('batch_ids', $enrolledBatchIds)) && in_array($batch->id, old('batch_ids', $enrolledBatchIds))) ? 'selected' : '' }}>
